...
class LogAnalyzer:
    """
    Token-efficient log analyzer for MADF execution traces (Sync version)
    ...
    def get_session_summary(self, session_id: str) -> str:
        ...
        """
        stats = self.pg.get_session_stats(session_id)
...
"""
    ...
    def compare_stories(self, story_id_1: str, story_id_2: str) -> str:
        """
        Compare two stories (<300 tokens)

        Returns side-by-side comparison with key metrics
        """
        comparison = self.pg.get_story_comparison(story_id_1, story_id_2)
...
"""
    ...
    def get_top_errors(self, limit: int = 5) -> str:
        """
        Get top recurring errors (<250 tokens)

        Returns most common error patterns with occurrence counts
        """
        patterns = self.pg.find_error_patterns(min_occurrences=2)
    ...
    def get_agent_performance(self, agent_name: str) -> str:
        """
        ...
        results = self.pg.execute_query(query, {"agent_name": agent_name})
...
"""
    ...
    def get_workflow_insights(self, workflow_id: str) -> str:
        """
        ...
        results = self.pg.execute_query(query, {"workflow_id": workflow_id})
    ...
    def get_token_usage_report(self, story_id: str) -> str:
        """
        ...
        results = self.pg.execute_query(query, {"story_id": story_id})
